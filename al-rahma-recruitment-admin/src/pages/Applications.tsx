...
function buildTimeline(application: ApplicationRecord) {
  const items = [
    ...
    },
    {
      ...
      helper: application.respondedAt ? `آخر تحديث ${formatRelativeTime(application.respondedAt)}` : 'بانتظار القرار',
...
export default function Applications() {
  ...
  const visibleApplications = useMemo(() => {
    return state.applications
      .filter((application) => {
        ...
        return matchesQuery && matchesCompany && matchesJob && matchesStatus;
      })
      .sort((first, second) => {
  ...
  }, [companyFilter, jobFilter, query, state.applications, statusFilter]);
  ...
  return (
    <>
      ...
      <section className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-7 text-[#64768a]">
            كل طلب هنا له رقم واضح، حالة قابلة للتتبع، سبب رفض عند الحاجة، وملاحظات داخلية لا تظهر إلا داخل لوحة الأدمن.
          </p>
        </div>
      </section>
      ...
      <AdminDataShell
        toolbar={
          <div className="grid gap-3 xl:grid-cols-[220px_220px_200px_minmax(0,1fr)]">
        ...
        {visibleApplications.length ? (
                ...
                >
                  ...
                  <div className="mt-4 grid gap-3 sm:grid-cols-2">
                    <div className="rounded-[1rem] bg-[#f7f9fc] px-3.5 py-3">
                      <div className="text-[11px] font-bold text-[#7a8b9e]">الوظيفة</div>
                      <div className="mt-2 text-sm font-black text-[#122341]">{cleanAdminText(application.jobTitle || 'غير محدد')}</div>
                    </div>
                    <div className="rounded-[1rem] bg-[#f7f9fc] px-3.5 py-3">
                      <div className="text-[11px] font-bold text-[#7a8b9e]">الشركة</div>
                      <div className="mt-2 text-sm font-black text-[#122341]">{cleanAdminText(application.companyName || 'غير محددة')}</div>
                    </div>
                  </div>
          ...
          <div className="p-6">
            <AdminEmptyState
              ...
              description="جرّب تعديل الفلاتر الحالية أو انتظر وصول طلبات جديدة من الموقع."
            />
          </div>
        )}
      </AdminDataShell>
      ...
      <AdminDrawer
        ...
        onClose={closeDrawer}
        title="ملف المتقدم"
        description="عرض كامل لبيانات الطلب، خط الحالة، الملاحظات الداخلية، والقرار النهائي."
      >
        {selectedApplication ? (
          <div className="space-y-5">
            <div className="space-y-2 text-right">
              ...
              <div className="flex flex-wrap gap-2">
                <AdminBadge tone={getStatusTone(selectedApplication.status) as 'success' | 'warning' | 'danger' | 'neutral'}>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {[
                ...
                ['المحافظة', selectedApplication.governorate || 'غير محدد'],
              ...
              <div className="grid gap-4">
                ...
                <AdminField label="سبب الرفض">
                  <AdminTextarea
                    ...
                    placeholder="اكتب سببًا واضحًا يظهر عند الرفض..."
                  />
                </AdminField>

                <AdminButton className="w-full" onClick={saveStatus}>
                  حفظ قرار الطلب
                </AdminButton>
              </div>
            </section>
            ...
            <section className="rounded-[1.2rem] border border-[rgba(24,37,63,0.08)] bg-white p-4">
              ...
              <div className="grid gap-3">
                <AdminTextarea
                  ...
                  placeholder="أضف ملاحظة إدارية عن هذا الطلب..."
                />
