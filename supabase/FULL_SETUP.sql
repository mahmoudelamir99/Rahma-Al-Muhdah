-- ============================================================
-- FULL DATABASE SETUP - منصة الرحمة المهداة
-- انسخ كل هذا الملف وشغّله في Supabase SQL Editor
-- يعمل بأمان على قاعدة بيانات فارغة أو موجودة (IF NOT EXISTS)
-- ============================================================
...
language sql security definer set search_path = public as $$
  select
    a.request_id, a.applicant_name, a.job_title, a.company_name,
    a.status, a.rejection_reason, a.submitted_at, a.responded_at
  from public.applications a
  join public.jobs j on j.id = a.job_id
  join public.companies c on c.id = a.company_id
  where a.request_id = p_request_id
    and a.deleted_at is null and j.deleted_at is null and c.deleted_at is null
    and j.status = 'approved' and c.status = 'approved'
  limit 1;
$$;

create or replace function public.track_application_by_tracking_id(p_tracking_id text)
returns table (
  tracking_id text, request_id bigint, job_title text, company_name text,
  location text, status text, rejection_reason text,
  submitted_at timestamptz, responded_at timestamptz
)
language sql stable security definer set search_path = public as $$
  select
    coalesce(a.tracking_id, a.request_id::text) as tracking_id,
    a.request_id, a.job_title, a.company_name,
...
returns table (can_submit boolean, attempt_number integer, message text)
language sql stable security definer set search_path = public as $$
  with matching as (
    select a.status from public.applications a
    where a.job_id = p_job_id and coalesce(a.national_id, '') = p_national_id and a.deleted_at is null
  ),
  stats as (
    select
  ...
  select
    case when active_count > 0 then false when attempts_count = 0 then true when attempts_count = 1 and rejected_count = 1 then true else false end,
    case when attempts_count = 0 then 1 when attempts_count = 1 and rejected_count = 1 then 2 else greatest(attempts_count, 1) end,
    case
...
grant execute on function public.track_application_by_request(bigint) to anon, authenticated;
...
-- Policy: أي authenticated user يقدر يرفع في مجلد اسمه
...
-- ─── انتهى الإعداد ─────────────────────────────────────────
-- بعد تشغيل هذا الملف، قم بتشغيل SET_ADMIN.sql لإنشاء حساب الأدمن
